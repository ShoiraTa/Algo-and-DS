class Gadget 
     ...
     # @username 
     # end
     def initialize(username, password) #create instance variables
     ...
     def gadget_info #create instance method
          "Production number is  #{@production_number} has password #{@password} "
     end


     def to_s #owerwriting to_s method 
           "username: #{ @username} ,password:  #{@password } "
     end  


     def self_keyword #self key word 
           "class is #{self.class} its id is #{self.object_id} "
     end 
...
#      end
     ...
     def generate_production_number
         number =  "private method- #{("a".."z").to_a.sample}- #{rand(100..199)}"
         number
     end
