...
friends.sort_by { |_key, val| val } #=> [[:Mohigul, "female"], [:Adham, "male"]]
...
friends.map { |_key, val| val.upcase } #=> ["FEMALE", "MALE"]
friends.select { |_name, gen| gen == 'female' }  #=>  {:Mohigul=>"female"}
friends.reject { |_name, gen| gen == 'female' }  #=>  {:Adham=>"male"}
